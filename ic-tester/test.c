...
bool TestData(uint8_t **ptr) {
	...
	for (;;) {
		...
		switch (cmd) {
			...
			case CMD_SET_16:			// (mask0[2], mask1[2]) выставл€ет логические 0 и 1 на выводах по маскам
				...
				break;
			case CMD_SET_28:			// (mask0[3], mask1[3]) выставл€ет логические 0 и 1 на выводах по маскам
				{
	...
	MSG("fail");
				...
				break;
			case CMD_TEST_28:			// (mask0[3], mask1[3]) провер€ет, что на выводах установлен ожидаемый уровень
				{
					uint32_t mask0 = pgm_read_3bytes(*ptr);
					*ptr += 3;
					uint32_t mask1 = pgm_read_3bytes(*ptr);
